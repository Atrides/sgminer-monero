...
{
	//if (!strcmp(arg, CRYPTONIGHT_KERNNAME))
	//	return KL_CRYPTONIGHT;
	return KL_CRYPTONIGHT;
}

void set_cryptonight_kernel()
{
...
retry: // TODO: refactor
	...
	if (!strncasecmp(&input, "e", 1)) {
		...
		//if ( (gpus[selected].kernel == KL_CRYPTONIGHT) ) {
			for (i = 0; i < mining_threads; ++i) {
				thr = get_thread(i);
				cgpu = thr->cgpu;
				if (cgpu->drv->drv_id != DRIVER_opencl)
...
static cl_int queue_cryptonight_kernel(_clState *clState, dev_blk_ctx *blk, __maybe_unused cl_uint threads)
{
	...
	flip76(clState->cldata, blk->work->data);
...
static bool opencl_thread_prepare(struct thr_info *thr)
{
	...
	if (!cgpu->kname)
	{
		cgpu->kname = CRYPTONIGHT_KERNNAME;
	}
	applog(LOG_INFO, "initCl() finished. Found %s", name);
	cgtime(&now);
	get_datestamp(cgpu->init, sizeof(cgpu->init), &now);
...
static bool opencl_thread_init(struct thr_info *thr)
{
	...
	thrdata->queue_kernel_parameters = &queue_cryptonight_kernel;
