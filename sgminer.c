...
static double get_work_coindiff(const struct work *work)
{
	...
	uint32_t diff32 = be32toh(*((uint32_t *)(work->data + 72))) & 0xFFFFFFFF; ///?????????72
...
static bool submit_upstream_work(struct work *work, CURL *curl, bool resubmit)
{
	...
	if (work->gbt) {
		...
		flip76(data, work->data);
		gbt_block = bin2hex(data, 76);
...
static const double truediffone = 115792089237316195423570985008687907853269984665640564039457584007913129639935.0;
...
static bool stale_work(struct work *work, bool share)
{
	...
	if (!share && pool->has_stratum) {
		...
		if (strcmp(work->job_id, pool->swork2.job_id))
			same_job = false;
...
static void discard_stale(void)
{
	...
	int stale = 0;
	applog(LOG_DEBUG, "Discard stale work");
	mutex_lock(stgd_lock);
	HASH_ITER(hh, staged_work, work, tmp) {
...
void write_config(FILE *fcfg)
{
	...
	if (nDevs) {
		...
		for(i = 0; i < nDevs; i++) {
			fprintf(fcfg, "%s", i > 0 ? "," : "");
			fprintf(fcfg, CRYPTONIGHT_KERNNAME);
		}
...
static void *stratum_rthread(void *userdata)
{
	...
	while (42) {
		...
		if (!parse_method(pool, s) && !parse_stratum_response(pool, s))
			...
			struct work *work = make_work();
			applog(LOG_INFO, "gen strat work rthr");
...
static void *stratum_sthread(void *userdata)
{
	...
	while (42) {
		...
		unsigned char	hash2[128];
		cryptonight_hash_cpu(hash2,work->data, 76);
		char *hashhex2 = bin2hex((const unsigned char *)hash2, 32);
		if (hashhex2 != hashhex) {
			applog(LOG_ERR, "DIFFERENT_HASH Here: %s  Mustbe: %s", hashhex, hashhex2);
			free_work(work);
			continue;
		}
		...
		*nonce2_64 = htole64(work->nonce2);
		...
		snprintf(s, sizeof(s),
			"{\"params\": [\"%s\", \"%s\", \"%s\", \"%s\"], \"id\": %d, \"method\": \"mining.submit\"}",
			pool->user_id, work->job_id, noncehex, hashhex, sshare->id);
...
void set_target_old(unsigned char *dest_target, double diff)
{
	unsigned char target[32];
	uint64_t *data64, h64;
	double d64, dcut64;
...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
	...
	memcpy(work, &pool->swork2, sizeof(struct work));
	if( pool->swork.job_id ) strncpy(work->job_id, pool->swork2.job_id, 128);
	
	/* Store the stratum work diff to check it still matches the pool's
	 * stratum diff when submitting shares */
	work->sdiff = pool->swork2.sdiff; //?????????????????????

	/* Copy parameters required for share submission */
	work->job_id = strdup(pool->swork2.job_id);
	cg_runlock(&pool->data_lock);
...
retry:
		if (pool->has_stratum) {
			...
			//pool->swork2.job_id;
    //applog(LOG_DEBUG, "job_id: %s", pool->swork2.job_id);
	//applog(LOG_DEBUG, "data: %s", bin2hex((const unsigned char *)pool->swork2.data, 128));
    //applog(LOG_DEBUG, "target: %s", bin2hex((const unsigned char *)pool->swork2.target, 32));
			gen_stratum_work(pool, work);
