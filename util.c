...
static unsigned char *rpc1_blob = NULL;
static int rpc1_bloblen = 0;
//static double rpc1_target = 0;
static uint32_t rpc1_target = 0;
static char *rpc1_job_id = NULL;
...
static bool parse_notify2(struct pool *pool, json_t *val)
{
    char *user_id, *job_id, *hexblob, *hextarget;   /*   *prev_hash, *coinbase1, *coinbase2, *bbversion, *nbit, *ntime, *header */;
    size_t cb1_len, cb2_len, alloc_len;
    unsigned char *cb1, *cb2;
    bool clean, ret = false;
    int i;

    user_id = json_array_string(val, 0);
    job_id = json_array_string(val, 1);
    hexblob = json_array_string(val, 2);
    hextarget = json_array_string(val, 3);

    if (!job_id || !hexblob || !hextarget ) {
    ...
    pool->user_id = strdup(user_id);
    int blobLen = strlen(hexblob);
    if (blobLen == 0 || blobLen % 2 != 0 || ((blobLen / 2) < 40 && blobLen != 0) || (blobLen / 2) > 128) {
    ...
    uint32_t target;
    hex2bin((unsigned char*)&target, hextarget, 4);
    if(rpc1_target != target) {
            double hashrate = 0.;
            /*cg_wlock(&pool->data_lock); stats_lock 
            for (int i = 0; i < opt_n_threads; i++)
                hashrate += thr_hashrates[i];
            cg_wunlock(&pool->data_lock);*/
            double difficulty = (((double) 0xffffffff) / target);
            applog(LOG_INFO, "Pool set diff to %g", difficulty);
            pool->swork2.sdiff = target;
            rpc1_target = target;
    }
    ...
    rpc1_bloblen = blobLen / 2;
    rpc1_blob = (char *)malloc(rpc1_bloblen);

    memcpy(rpc1_blob, blob, blobLen / 2);
    free(blob);

    if (pool->swork2.job_id) {
        free(pool->swork2.job_id);
	}

    memcpy(pool->swork2.data, rpc1_blob, rpc1_bloblen);
	//set_target(pool->swork2.target, rpc1_target);
	/*memset(pool->swork2.target, 0xff, sizeof(pool->swork2.target));
    pool->swork2.target[7] = rpc1_target;*/
    if (rpc1_job_id) {
            free(rpc1_job_id);
    }
    rpc1_job_id = strdup(job_id);

	pool->swork2.job_id = strdup(job_id);
    //strncpy(pool->swork2.job_id, rpc1_job_id, 128);

	//memset(pool->swork2.wtarget, 0xff, sizeof(pool->swork2.wtarget));
	//pool->swork2.wtarget[7] = rpc1_target;

    //applog(LOG_DEBUG, "parse_notify2 job_id: %s", pool->swork2.job_id);
	//applog(LOG_DEBUG, "parse_notify2 data: %s", bin2hex((const unsigned char *)pool->swork2.data, 128));
	//applog(LOG_DEBUG, "parse_notify2 swork2.sdiff: %g", pool->swork2.sdiff);
	//applog(LOG_DEBUG, "parse_notify2 target: %i", rpc1_target);
	//applog(LOG_DEBUG, "parse_notify2 wtarget: %08lX  %" PRIu32, (unsigned long) pool->swork2.wtarget, pool->swork2.wtarget[7]);
	
	//unsigned char	starget[32];
	//set_target(starget, rpc1_target);
	set_target(pool->swork2.target, (double)rpc1_target);
	//snprintf(pool->swork2.target+28, 4, "%lu", (unsigned long)rpc1_target);
    applog(LOG_DEBUG, "parse_notify2 target: %s", bin2hex((const unsigned char *)pool->swork2.target, 32));
...
static bool parse_diff(struct pool *pool, json_t *val)
{
	...
	cg_wlock(&pool->data_lock);
	old_diff = pool->swork2.sdiff;
	pool->swork2.sdiff = diff;
	cg_wunlock(&pool->data_lock);
